...
- **Filter** uses `setpts=PTS/<speed>` on the video stream and `-an` to drop audio entirely. Do not switch to `atempo` / re-add audio without changing the spec — the README explicitly states "mute".
